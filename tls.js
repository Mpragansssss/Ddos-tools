...
if (process.argv.length < 7) {
    ...
    console.log(`
    ${'方法 & 示例'.bold}:
        node tls.js [target] [time] [rate] [threads] [proxyfile]
        node tls.js https://example.com 120 64 10 proxy.txt --debug true

    ${'更多选项'.bold}:
        --debug         ${'true'.green}/${'false'.red}   ~   ${'启用调试模式'.italic}
        --redirect      ${'true'.green}/${'false'.red}   ~   ${'启用重定向系统'.italic}
        --ratelimit     ${'true'.green}/${'false'.red}   ~   ${'启用速率限制系统'.italic}
        --query         ${'true'.green}/${'false'.red}   ~   ${'启用随机查询系统'.italic}
    `);
    process.exit(0);
}
...
function _tlsSocket(parsed, socket) {
    ...
    tls_socket.setKeepAlive(true, 5000 * 1000);
                            ...
                            req.on("response", (res) => {
                                ...
                                if (enabled('debug')) {
                                    if (headers["cookie"] !== undefined) {
                                        console.log(`[${'+'.bold.magenta}] | ${colors.cyan(`${proxy[0]}:${proxy[1]}`.underline)}, ${headers[':authority']}${headers[':path']}, ${`${headers['cookie']}`.green.underline}, ${'状态'.bold}: ${coloredStatus}`);
                                    } else {
                                        console.log(`[${'+'.bold.magenta}] | ${colors.cyan(`${proxy[0]}:${proxy[1]}`.underline)}, ${headers[':authority']}${headers[':path']}, ${'状态'.bold}: ${coloredStatus}`);
                                    }
                                }
                                ...
                                if (status === 429 && enabled('ratelimit')) {
                                    ...
                                    if (enabled('redirect')) {
                                        if (parsed.href !== redirectedParsed.href) {
                                            if (enabled('debug')) {
                                                console.log(`[${'+'.bold.magenta}] | ${colors.cyan.underline(proxy)} ${'重定向'.bold}: ${parsed.href} -> ${redirectedParsed.href}`)
                                            }
...
if (cluster.isMaster){
    console.clear();
    console.log(`-`.red, '目标:'.bold, '['.red, `${target}`.underline, ']'.red);
    console.log("-".red, "时间:".bold, "[".red, `${duration}`.underline, "]".red);
    console.log("-".red, "线程:".bold, "[".red, `${threads}`.underline, "]".red);
    console.log("-".red, "速率:".bold, "[".red, `${rate}`.underline, "]".red);
    console.log("-".red, "文件:".bold, "[".red, `${proxyfile}`.underline, "]".red);
